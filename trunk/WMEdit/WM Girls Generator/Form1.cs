...
namespace WM_Girls_Generator
{
    public partial class Form1 : Form
    {
        ...
        private string[] CompileGirl()
        {
            ...
            sSkills = SkillTBox_01.Text + sp + SkillTBox_02.Text + sp + SkillTBox_03.Text + sp + SkillTBox_04.Text + sp + SkillTBox_05.Text + sp + SkillTBox_06.Text + sp + SkillTBox_07.Text + sp + SkillTBox_08.Text + sp + SkillTBox_09.Text + sp + SkillTBox_10.Text;
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            ...
            SkillTBox_01.Text = ""; SkillTBox_02.Text = ""; SkillTBox_03.Text = ""; SkillTBox_04.Text = ""; SkillTBox_05.Text = ""; SkillTBox_06.Text = ""; SkillTBox_07.Text = ""; SkillTBox_08.Text = ""; SkillTBox_09.Text = ""; SkillTBox_10.Text = "";
        ...
        private void LoadGirlsXML(string path)
        {
            try
            {
                ...
                foreach (XmlNode node in baseNode.SelectNodes("/Girls/Girl"))       //Here we tell it what node we want, basically get collection of all Girl nodes in Girls node, in original XML "Girls" is root node, and each girl has her "Girl" node
                {
                    ...
                    string sName = node.Attributes["Name"].Value;                   //girl name, each girl value has it's attribute within it's node, this is how they're read
                    ...
                    string sGold = node.Attributes["Gold"].Value;                   //starting gold
                    ...
                    sData = node.Attributes["Charisma"].Value + " " + node.Attributes["Happiness"].Value + " " + node.Attributes["Libido"].Value + " " + node.Attributes["Constitution"].Value + " " + node.Attributes["Intelligence"].Value + " " + node.Attributes["Confidence"].Value + " " + node.Attributes["Mana"].Value + " " + node.Attributes["Agility"].Value + " " + node.Attributes["Fame"].Value + " " + node.Attributes["Level"].Value + " " + node.Attributes["AskPrice"].Value + " " + node.Attributes["House"].Value + " " + node.Attributes["Exp"].Value + " " + node.Attributes["Age"].Value + " " + node.Attributes["Obedience"].Value + " " + node.Attributes["Spirit"].Value + " " + node.Attributes["Beauty"].Value + " " + node.Attributes["Tiredness"].Value + " " + node.Attributes["Health"].Value + " " + node.Attributes["PCFear"].Value + " " + node.Attributes["PCLove"].Value + " " + node.Attributes["PCHate"].Value;
                    sSkills = node.Attributes["Anal"].Value + " " + node.Attributes["Magic"].Value + " " + node.Attributes["BDSM"].Value + " " + node.Attributes["NormalSex"].Value + " " + node.Attributes["Beastiality"].Value + " " + node.Attributes["Group"].Value + " " + node.Attributes["Lesbian"].Value + " " + node.Attributes["Service"].Value + " " + node.Attributes["Strip"].Value + " " + node.Attributes["Combat"].Value;
                    ...
                    if (node.HasChildNodes == true)                 //next are attributes, they're child node of parent "Girl" node, simply check if there are any, if yes go through all of them to store each one in alTraits ArrayList
                    {
                    ...
                    string sTraits = alTraits.Count.ToString();     //old format requires that there's a number of traits, this is it. This string will store all traits, so first we put number of traits in it
 ...
 */
        ...
        {                                                                   //and this is other part of the wrapper, this one reads old format from memory and stores it in XML
            ...
            for (int x = 0; x < GirlsCollection.Rows.Count; x++)            //and now we need to go through each girl, i.e. row in DataTable
            {
                ...
                string sStatus = sData.ReadLine();                          //status, slave, normal, catacombs
        ...
        private void GirlChanged(ref DataTable dt)
        {
            ...
            StatsTBox_01.Text = values[0];                                          //and now fill stats boxes with values from previous array
            StatsTBox_02.Text = values[1];
            StatsTBox_03.Text = values[2];
            StatsTBox_04.Text = values[3];
            StatsTBox_05.Text = values[4];
            StatsTBox_06.Text = values[5];
            StatsTBox_07.Text = values[6];
            StatsTBox_08.Text = values[7];
            StatsTBox_09.Text = values[8];
            StatsTBox_10.Text = values[9];
            StatsTBox_11.Text = values[10];
            StatsTBox_12.Text = values[11];
            StatsTBox_13.Text = values[12];
            StatsTBox_14.Text = values[13];
            StatsTBox_15.Text = values[14];
            StatsTBox_16.Text = values[15];
            StatsTBox_17.Text = values[16];
            StatsTBox_18.Text = values[17];
            StatsTBox_19.Text = values[18];
            StatsTBox_20.Text = values[19];
            StatsTBox_21.Text = values[20];
            StatsTBox_22.Text = values[21];

            values = data.ReadLine().Split(separator);                              //again for skills

            SkillTBox_01.Text = values[0];
            SkillTBox_02.Text = values[1];
            SkillTBox_03.Text = values[2];
            SkillTBox_04.Text = values[3];
            SkillTBox_05.Text = values[4];
            SkillTBox_06.Text = values[5];
            SkillTBox_07.Text = values[6];
            SkillTBox_08.Text = values[7];
            SkillTBox_09.Text = values[8];
            SkillTBox_10.Text = values[9];

            GoldTBox1.Text = data.ReadLine();                                       //gold

            string gtype = data.ReadLine();                                         //and girl type

            if (Convert.ToInt32(gtype) < 0 || Convert.ToInt32(gtype) > 2)           //Originaly there wasn't an "if" planned here, which works fine if input data is correct, but in the case it's out of bounds you get an error, so to prevent these from scaring users this if defaults the droplist to "Normal Girl" if value is out of bounds
            {
                comboBox_GirlType.SelectedIndex = 0;
