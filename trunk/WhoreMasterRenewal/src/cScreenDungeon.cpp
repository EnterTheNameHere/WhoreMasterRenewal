 ...
 */
		if (selected_girl == girl)
			selection = i;
/*
 *		if she's low health, flag her entry to display in red
 */
		int col = (girl->health() <= 30) ? LISTBOX_RED : LISTBOX_BLUE;
/*
 *		add her to the list
 */
 ...
 *	can't ... resist ...
 */
	g_MessageQue.AddToQue("This is a dead girl. She has ceased to be.", 1);
/*
 ...
 */
		...
		if(ggf.girl_submits()) {
			...
			message += " submits the the enchanted slave tattoo"
 ...
 */
			...
			set_slave_stats(girl);
			message += girl->m_Realname;
			message += " breaks free from your goons' control. "
				   "You restrain her personally while the "
		...
		if(ggf.girl_lost()) {
			message += girl->m_Realname;
			message += " puts up a fight ";
/*
 *			there was a gang, and some of them are still with us
 */
			message += "but your goons control her as the "
					   "enchanted slave tattoo is placed upon her."
			;
/*
