    ...
    m_value(0.0)
{
    if( init_val != 0 )
    {
        m_initial_value = static_cast<double>( init_val );
        m_value = static_cast<double>( init_val );
    }
    else
    {
        cConfig cfg;
...
int cGoldBase::ival()
{
    return static_cast<int>( floor( m_value ) );
}
...
cGold::cGold() : cGoldBase()
{
    ;
}

cGold::cGold( int initial ) : cGoldBase( initial )
{
    ;
}
...
int cGold::total_income()
{
    ...
    return static_cast<int>( floor( sum ) );
}
...
int cGold::total_profit()
{
    return total_earned() - total_upkeep();
}
