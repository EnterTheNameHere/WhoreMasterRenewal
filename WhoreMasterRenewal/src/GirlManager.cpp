...
bool GirlManager::CheckInvSpace( Girl* girl )
{
...
void GirlManager::SetAntiPreg( Girl* girl, bool useAntiPreg )
{
    girl->m_UseAntiPreg = useAntiPreg;
}

void GirlManager::Free()
{
    if( m_Parent )
        delete m_Parent;
...
bool GirlManager::DisobeyCheck( Girl* girl, int action, Brothel* brothel )
{
    int diff;
...
void GirlManager::CalculateGirlType( Girl* girl )
{
    int BigBoobs = 0;
    int CuteGirl = 0;
    int Dangerous = 0;
...
bool GirlManager::CheckGirlType( Girl* girl, int type )
{
...
void GirlManager::CalculateAskPrice( Girl* girl, bool vari )
{
...
sRandomGirl* GirlManager::random_girl_at( u_int n )
{
    u_int i;
    ...
    for( i = 0; i < n; i++ )
    {
        ...
        if( current == nullptr )
        {
            g_LogFile.ss() << "broken chain in GirlManager::random_girl_at" << std::endl;
            g_LogFile.ssend();
            return nullptr;
        }
    }
...
Girl* GirlManager::CreateRandomGirl( int age, bool addToGGirls, bool slave, bool /*undead*/, bool NonHuman, bool childnaped )
{
    cConfig cfg;
    sRandomGirl* current;
    ...
    for( int i = 0; i < current->m_NumTraits; i++ ) // add the traits
    {
        ...
        {
            std::string message = "GirlManager::CreateRandomGirl: ERROR: Trait '";
...
bool GirlManager::NameExists( std::string name )
{
...
void GirlManager::LevelUp( Girl* girl )
{
    std::stringstream ss;
...
void GirlManager::LevelUpStats( Girl* girl )
{
    /*
    int DiceSize = 0;
...
void GirlManager::AddRandomGirl( sRandomGirl* girl )
{
    girl->m_Next = nullptr;
...
void GirlManager::AddGirl( Girl* girl )
{
    girl->m_Prev = nullptr;
    girl->m_Next = nullptr;
...
void GirlManager::RemoveGirl( Girl* girl, bool deleteGirl )
{
    if( m_Parent == nullptr )
        return;
...
void GirlManager::AddTiredness( Girl* girl )
{
...
int GirlManager::GetSlaveGirl( int from )
{
    Girl* current = m_Parent;
    int num = 0;
    int girlnum = 0;
...
std::vector<Girl*> GirlManager::get_girls( GirlPredicate* pred )
{
    Girl* girl;
    std::vector<Girl*> v;
...
std::string GirlManager::GetGirlMood( Girl* girl )
{
...
std::string GirlManager::GetMoreDetailsString( Girl* girl )
{
    if( girl == nullptr )
        return std::string( "" );
...
std::string GirlManager::GetDetailsString( Girl* girl, bool purchase )
{
    cConfig cfg;
    cTariff tariff;
    std::stringstream ss;
...
Girl* GirlManager::GetRandomGirl( bool slave, bool catacomb )
{
...
Girl* GirlManager::GetGirl( int girl )
{
    int count = 0;
    Girl* current = m_Parent;
...
int GirlManager::GetRebelValue( Girl* girl, bool matron )
{
    /*
...
int GirlManager::GetNumCatacombGirls()
{
    int number = 0;
    Girl* current = m_Parent;
...
int GirlManager::GetNumSlaveGirls()
{
    int number = 0;
    Girl* current = m_Parent;
...
int GirlManager::GetStat( Girl* girl, int a_stat )
{
    u_int stat = a_stat;
    int value = 0;
...
void GirlManager::SetStat( Girl* girl, int a_stat, int amount )
{
    u_int stat = a_stat;
...
void GirlManager::UpdateStat( Girl* girl, int a_stat, int amount )
{
    u_int stat = a_stat;
...
void GirlManager::updateTempStats( Girl* girl )
{
    /*
...
void GirlManager::UpdateTempStat( Girl* girl, int stat, int amount )
{
    if( amount >= 0 )
    {
...
int GirlManager::GetSkill( Girl* girl, int skill )
{
...
void GirlManager::SetSkill( Girl* girl, int skill, int amount )
{
    girl->m_SkillMods[skill] = amount;
}

int GirlManager::GetSkillWorth( Girl* girl )
{
    int num = 0;
...
void GirlManager::UpdateSkill( Girl* girl, int skill, int amount )
{
    if( amount >= 0 )
    {
...
void GirlManager::updateTempSkills( Girl* girl )
{
    /*
     *   Normalise to zero by 30%
     */
...
void GirlManager::UpdateTempSkill( Girl* girl, int skill, int amount )
{
    if( amount >= 0 )
    {
...
void GirlManager::LoadGirlLegacy( Girl* current, std::ifstream& ifs )
{
    cConfig cfg;
    int temp = 0;
    std::stringstream ss;
...
void GirlManager::LoadRandomGirl( std::string filename )
{
    /*
...
void GirlManager::LoadRandomGirlXML( std::string filename )
{
...
void GirlManager::LoadRandomGirlLegacy( std::string filename )
{
    std::ifstream in;
...
void GirlManager::LoadGirlsDecider( std::string filename )
{
    /*
     *  before we go any further: files that end in "x" are
     *  in XML format. Get the last char of the filename.
...
void GirlManager::LoadGirlsXML( std::string filename )
{
    cConfig cfg;
...
void GirlManager::LoadGirlsLegacy( std::string filename )
{
    std::ifstream in;
    in.open( filename.c_str() );
    char buffer[500];
    ...
    while( in.good() )
    {
        ...
        for( int i = 0; i < tempData; i++ ) // load in the traits
        {
            ...
            if( g_Traits.GetTrait( buffer ) == nullptr ) // test a trait exists
            {
                std::string message = "ERROR in GirlManager::LoadGirls: Trait '";
                message += buffer;
                message += "' from girl template ";
                message += newGirl->m_Name;
                message += " doesn't exist or is spelled incorrectly.";
...
bool GirlManager::LoadGirlsXML( TiXmlHandle hGirls )
{
...
void GirlManager::LoadGirlsLegacy( std::ifstream& ifs )
{
    Girl* current = nullptr;
    int temp;
...
TiXmlElement* GirlManager::SaveGirlsXML( TiXmlElement* pRoot )
{
...
int GirlManager::HasItem( Girl* girl, std::string name )
{
    for( int i = 0; i < 40; i++ )
    {
...
void GirlManager::EquipCombat( Girl* girl )
{
...
void GirlManager::UnequipCombat( Girl* girl )
{
...
void GirlManager::UseItems( Girl* girl )
{
    bool withdraw = false;
...
bool GirlManager::CanEquip( Girl* girl, int num, bool force )
{
    if( force )
        return true;
...
bool GirlManager::IsItemEquipable( Girl* girl, int num )
{
    switch( girl->m_Inventory[num]->m_Type )
    {
...
bool GirlManager::EquipItem( Girl* girl, int num, bool force )
{
...
bool GirlManager::IsInvFull( Girl* girl )
{
    bool full = true;
...
int GirlManager::AddInv( Girl* girl, sInventoryItem* item )
{
    int i;
...
bool GirlManager::RemoveInvByNumber( Girl* girl, int Pos )
{
...
void GirlManager::SellInvItem( Girl* girl, int num )
{
...
int GirlManager::GetWorseItem( Girl* girl, int type, int cost )
{
    int ret = -1;
...
int GirlManager::GetNumItemType( Girl* girl, int Type )
{
    if( girl->m_NumInventory == 0 )
        return 0;
...
int GirlManager::GetNumItemEquiped( Girl* girl, int Type )
{
    if( girl->m_NumInventory == 0 )
        return 0;
...
void GirlManager::UnapplyTraits( Girl* girl, sTrait* trait )
{
    /* WD:
...
bool GirlManager::PossiblyGainNewTrait( Girl* girl, std::string Trait, int Threshold, int ActionType, std::string Message, bool /*DayNight*/ )
{
...
void GirlManager::ApplyTraits( Girl* girl, sTrait* trait, bool rememberflag )
{
    /* WD:
    *   Added doOnce = false; to end of fn
    *   else the fn will allways abort
...
bool GirlManager::HasRememberedTrait( Girl* girl, std::string trait )
{
...
bool GirlManager::HasTrait( Girl* girl, std::string trait )
{
...
void GirlManager::RemoveRememberedTrait( Girl* girl, std::string name )
{
...
void GirlManager::RemoveAllRememberedTraits( Girl* girl )
{
    /*
...
bool GirlManager::RemoveTrait( Girl* girl, std::string name, bool addrememberlist, bool force )
{
    /*
...
void GirlManager::AddRememberedTrait( Girl* girl, std::string name )
{
...
bool GirlManager::AddTrait( Girl* girl, std::string name, bool temp, bool removeitem, bool inrememberlist )
{
    /*
     *  WD: Added logic for remembered trait
     *
...
void GirlManager::updateTempTraits( Girl* girl )
{
    /*
...
void GirlManager::updateHappyTraits( Girl* girl )
{
    /*
...
void GirlManager::GirlFucks( Girl* girl, int /*DayNight*/, sCustomer* customer, bool group, std::string& message, u_int& SexType )
{
    bool good = false;
...
std::string GirlManager::GetRandomSexString()
{
...
std::string GirlManager::GetRandomGroupString()
{
    int roll1 = 0, roll2 = 0, roll3 = 0, random = 0;
    std::string OutStr;
    /*char buffer[10];*/
...
std::string GirlManager::GetRandomBDSMString()
{
...
std::string GirlManager::GetRandomBeastString()
{
    int roll1 = 0, roll2 = 0, roll3 = 0, random = 0;
    /*char buffer[10];*/
    std::string OutStr;
...
std::string GirlManager::GetRandomLesString()
{
...
std::string GirlManager::GetRandomAnalString()
{
...
Uint8 GirlManager::girl_fights_girl( Girl* a, Girl* b )
{
    ...
    g_LogFile.ss()  << "ERROR: GirlManager::girl_fights_girl - Shouldn't reach the function bottom.";
    g_LogFile.ssend();
    
    return 0;
}

bool GirlManager::GirlInjured( Girl* girl, unsigned int unModifier )
{
...
int GirlManager::GetCombatDamage( Girl* girl, int CombatType )
{
    int damage = 5;
...
int GirlManager::TakeCombatDamage( Girl* girl, int amt )
{
...
void GirlManager::UpdateEnjoyment( Girl* girl, int whatSheEnjoys, int amount, bool wrapTo100 )
{
...
void GirlManager::updateGirlAge( Girl* girl, bool inc_inService )
{
    /*
...
void GirlManager::updateSTD( Girl* girl )
{
    /*
...
void GirlManager::updateGirlTurnStats( Girl* girl )
{
    /*
...
bool GirlManager::CalcPregnancy( Girl* girl, int chance, int type, unsigned char stats[NUM_STATS], unsigned char skills[NUM_SKILLS] )
{
...
int GirlManager::calc_abnormal_pc( Girl* mom, Girl* sprog, bool is_players )
{
    /*
...
bool GirlManager::child_is_grown( Girl* mom, sChild* child, std::string& summary, bool PlayerControlled )
{
    cConfig cfg;
    cTariff tariff;
    std::stringstream ss;
...
int GirlManager::num_images( Girl* girl, int image_type )
{
    return girl->m_GirlImages->m_Images[image_type].m_NumImages;
}

void GirlManager::UncontrolledPregnancies()
{
    Girl* current = m_Parent;
    std::string summary;
...
void GirlManager::HandleChildren( Girl* girl, std::string& summary, bool PlayerControlled )
{
    sChild* child;
...
bool GirlManager::child_is_due( Girl* girl, sChild* child, std::string& summary, bool PlayerControlled )
{
    cConfig cfg;
    cTariff tariff;
    std::stringstream ss;
...
bool GirlManager::InheritTrait( sTrait* trait )
{
...
void GirlManager::HandleChildren( Girl* girl, std::string summary )
{
    cConfig cfg;
...
void GirlManager::LoadGirlImages( Girl* girl )
{
    girl->m_GirlImages = g_Girls.GetImgManager()->LoadList( girl->m_Name );
}

void GirlManager::LoadDefaultImages()    // for now they are hard coded
{
    m_DefImages = m_ImgListManager.LoadList( "Default" );
}

bool GirlManager::IsAnimatedSurface( Girl* girl, int ImgType, int& img )
{
    while( 1 )
    {
        switch( ImgType )
        ...
        default:
            ...
            g_LogFile.ss()  << "GirlManager::IsAnimatedSurface: "
...
std::shared_ptr<CSurface> GirlManager::GetImageSurface( Girl* girl, int ImgType, bool random, int& img, bool gallery )
{
    while( 1 )
    {
        /*
...
cAnimatedSurface* GirlManager::GetAnimatedSurface( Girl* girl, int ImgType, int& img )
{
 ...
 */
int GirlManager::get_modified_image_type( Girl* girl, int image_type, int preg_type )
{
 ...
 * set
 */
int GirlManager::draw_with_default(
    Girl* girl,
    int x, int y,
...
int GirlManager::DrawGirl( Girl* girl, int x, int y, int width, int height, int ImgType, bool random, int img )
{
    bool preg = false;
