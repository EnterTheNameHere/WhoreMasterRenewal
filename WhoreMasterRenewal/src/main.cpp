...
public:
    ...
    void RunApplication()
    {
        GetWindowManager()->Run();
    }

private:
    static std::shared_ptr<WindowManager> m_WindowManager;
};
std::shared_ptr<WindowManager> Managers::m_WindowManager = nullptr;
...
{
    wmr::Logger() << "Initialisation of static resources done...\n" <<
                    "Loading Whore Master: Renewal...\n";
    
    try
    {
        wmr::Managers managers;
        
        managers.RunApplication();
    }
    ...
    wmr::Logger() << "Whore Master: Renewal finished...\n" <<
                    "Releasing static resources now...\n";
